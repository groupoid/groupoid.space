...
\text{north} : \Sigma A \\
\text{south} : \Sigma A \\
\text{merid} : (a : A) \to \text{north} \equiv \text{south}
\end{cases}
\]
\begin{lstlisting}[mathescape=true]
def $\Sigma$ (A: U) : U
...
For a type $B : \mathcal{U}$, points $n, s : B$, and a family of
paths $m : (a : A) \to \text{Path}_B (n, s)$, there exists a
map $\text{rec}_{\Sigma A} : \Sigma A \to B$ such that:
\[
\begin{cases}
...
Any two maps $h_1, h_2 : \Sigma A \to B$ are homotopic if they agree
...
The suspension of the 0-sphere $S^0$ (two points) yields the 1-sphere $S^1$.
If $A = S^0 = \{ \text{base}_0, \text{base}_1 \}$, then $\Sigma S^0$ has two
points $\text{north}$, $\text{south}$, and two paths $\text{merid}(\text{base}_0)$,
$\text{merid}(\text{base}_1)$, resembling the loop structure of $S^1$.
\end{example}
...
\text{po3} : (c : C) \to \text{po1 } (f c) \equiv \text{po2 } (g c)
\end{cases}
\]
\begin{lstlisting}
...
\begin{theorem} (Elimination)
For a type $D : \mathcal{U}$, maps $h_A : A \to D$, $h_B : B \to D$,
and a family of paths $h_C : (c : C) \to \text{Path}_D (h_A (f c), h_B (g c))$,
there exists a map $\text{pushoutRec} : \text{pushout } A B C f g \to D$ such that:
\[
\begin{cases}
...
\begin{definition} (n-Spheres)
Alternatively, the sphere can be defined iteratively as:

\begin{lstlisting}[mathescape=true]
def sphere : $\mathbb{N}$ $\to$ U := $\mathbb{N}\text{-iter}$ U $\mathbf{2}$ $\Sigma$
\end{lstlisting}

This iterative definition applies the suspension functor $\Sigma$ to the base type $\mathbf{2}$ (the 0-sphere) $n$ times to obtain $S^n$.
\end{definition}

\begin{example} (Sphere as a CW-Complex)
The $n$-sphere $S^n$ can be constructed as a CW-complex with one 0-cell and one $n$-cell:
...
The $\text{cell}$ constructor glues the boundary of the $n$-cell to the base point, resulting in the type $S^n$.
\end{example}
...
\text{spoke} : (f : S \to \text{hubSpokes } S A) \to (s : S) \to \text{hub } f \equiv f s \\
\text{hubEq} : (x, y : A) \to (p : S \to \text{Path}_A(x, y)) \to \text{base } x \equiv \text{base } y \\
\text{spokeEq} : (x, y : A) \to (p : S \to \text{Path}_A(x, y)) \to (s : S) \to \text{hubEq } x y p \equiv \text{base } (p s)
\end{cases}
\]
\begin{lstlisting}
