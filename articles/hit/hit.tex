...
існує відображення \( \text{Ind}_* : A * B \to C \), таке що:
\[
\begin{cases}
\text{Ind}_*(\text{joinl}(a)) = f(a) \\
\text{Ind}_*(\text{joinr}(b)) = g(b) \\
\text{Ind}_*(\text{join}(a,b,i)) = h(a,b,i) \\
\end{cases}
\]
\begin{lstlisting}
def Ind_* (A B C : U) (f : A -> C) (g : B -> C)
  ...
  : A * B -> C
 := split { joinl a -> f a | joinr b -> g b | join a b @ i -> h a b @ i }
\end{lstlisting}
\end{theorem}

\begin{theorem} (Обчислення).
Для всіх $z : A * B$, і предиката $P$ виконуються правила $\text{Ind}_*$ для всіх параметрів предиката $P$.
\end{theorem}
