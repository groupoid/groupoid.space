...
\text{spokeEq} : (x, y : A) \to (p : S \to x \equiv y) \to (s : S) \\
\indent \to \text{hubEq}(x,y,p) \equiv \text{cong}(\text{base}(p(s)))
\end{cases}
\]
\begin{lstlisting}[mathescape=true]
def $\odot$ (S A: U) : U
...
\begin{theorem} (Елімінація).
Для сімейства типів \( P : \text{HubSpokes} \, S \, A \to \mathcal{U} \),
відображень \( \text{pbase} : (x : A) \to P(\text{base} \, x) \),
...
\( \text{phubEq} : (x, y : A) \to (p : S \to \text{Path} \, A \, x \, y) \to \text{PathP} \, (<i> P(\text{hubEq} \, x \, y \, p \, @ \, i)) \, (\text{pbase} \, x) \, (\text{pbase} \, y) \),
\( \text{pspokeEq} : (x, y : A) \to (p : S \to \text{Path} \, A \, x \, y) \to (s : S) \to \text{PathP} \, (<i> P(\text{spokeEq} \, x \, y \, p \, s \, @ \, i)) \, (P(\text{hubEq} \, x \, y \, p)) \, (\text{pbase} \, (p \, s)) \),
існує відображення \( \text{hubSpokesInd} : (z : \text{HubSpokes} \, S \, A) \to P(z) \), таке що:
\[
\begin{cases}
\text{Ind}_\odot \, (\text{base} \, x) = \text{pbase} \, x \\
\text{Ind}_\odot \, (\text{hub} \, f) = \text{phub} \, f \\
\text{Ind}_\odot \, (\text{spoke} \, f \, s \, @ \, i) = \text{pspoke} \, f \, s \, @ \, i \\
\text{ind}_\odot \, (\text{hubEq} \, x \, y \, p \, @ \, i) = \text{phubEq} \, x \, y \, p \, @ \, i \\
\text{Ind}_\odot \, (\text{spokeEq} \, x \, y \, p \, s \, @ \, i) = \text{pspokeEq} \, x \, y \, p \, s \, @ \, i \\
\end{cases}
.
\]
\end{theorem}
...
є вищим індуктивним типом, який представляє об’єднання \( A \)
...
\begin{definition} (Формація).
Для точкових типів \( A, B : \text{pointed} \), Букет \( A \vee B : \mathcal{U} \).
\end{definition}

\begin{definition} (Конструктори).
Букет генерується наступною вищою індуктивною композиційною структурою:
\[
\vee :=
\begin{cases}
...
\begin{theorem} (Елімінація).
Для типу \( P : A \vee B \mathcal{U} \), відображень \( f : A.1 \to C \),
\( g : B.1 \to C \), і шляху \( p : \text{PathOverlue}(P,f(A.2),g (B.2)) \),
існує відображення \( \text{Ind}_\vee : A \vee B \to C \), таке що:
\[
\begin{cases}
...
\begin{theorem} (Елімінація).
Для сімейств типів \( P : \text{Smash } A \, B \to \mathcal{U} \),
їх точок \( \text{pbasel} : P(\text{basel}) \),
         \( \text{pbaser} : P(\text{baser}) \), відображєнь
...
існує відображення \( \text{Ind}_\wedge : (z : A \wedge B) \to P(z) \), таке що:
\[
\begin{cases}
...
\begin{theorem} (Обчислення).
Для сімейства типів \( P : A \wedge B \to \mathcal{U} \),
точок \( \text{pbasel} : P(\text{basel}) \),
\( \text{pbaser} : P(\text{baser}) \),
відображення \( \text{pproj} : (x : A.1) \to (y : B.1) \to P(\text{proj} \, x \, y) \),
і сімейства шляхів
...
відображення \( \text{Ind}_\wedge : (z : A \wedge B) \to P(z) \), задовольняє всі рівняння для всіх варіантів
параметрів предиката $P$:
\[
\begin{cases}
...
\begin{theorem} (Унікальність).
Для сімейства типів \( P : A \wedge B \to \mathcal{U} \),
і відображень \( h_1, h_2 : (z : A \wedge B) \to P(z) \),
якщо існують шляхи
...
то \( h_1 \equiv h_2 \), тобто існує шлях \( (z : A \wedge B) \to h_1(z) \equiv h_2(z) \).
\end{theorem}

\newpage
\subsection{З’єднання}
З’єднання двох типів \( A \) і \( B \), позначене \( A \Join B \),
...
\begin{definition} (Формація).
Для типів \( A, B : \mathcal{U} \), з’єднання \( A * B : \mathcal{U} \).
\end{definition}
...
і сімейства шляхів \( h : (a : A) \to (b : B) \to f(a) \equiv g(b) \)
існує відображення \( \text{Ind}_\Join : A \Join B \to C \), таке що:
\[
\begin{cases}
...
\begin{theorem} (Обчислення).
Для всіх $z : A \Join B$, і предиката $P$ виконуються правила $\text{Ind}_\Join$ для всіх параметрів предиката $P$.
\end{theorem}

\begin{theorem} (Унікальність).
Будь-які два відображення \( h_1, h_2 : A \Join B \to C \) є гомотопними,
...
n \geq 1: \|\Sigma^{n-1}(K1'(G.1,G.2.1))\|_{n}
\end{cases}
\]
\begin{lstlisting}[mathescape=true]
