...
\[
\begin{array}{ccc}
    a & \xrightarrow{\text{pcomp}} & c \\
    \uparrow^{\text{[i] a}} & & \uparrow^{q} \\
    a & \xrightarrow{p @ i} & b \\
\end{array}
\]
...
$$
\begin{array}{ccc}
    a & \xrightarrow{\text{p}} & b \\
    \uparrow^{\text{[i] a}} & & \uparrow^{q} \\
    a & \xrightarrow{\text{[i] a}} & a \\
\end{array}
$$
\begin{lstlisting}[mathescape=true]
...
\begin{lstlisting}
im1 (A B: U) (f: A -> B): U = (b: B) * pTrunc ((a:A) * Ξ B (f a) b)
