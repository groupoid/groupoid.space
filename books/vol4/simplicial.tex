...
\newpage
\subsection*{Approach Analysis: Modal STT vs. Synthetic $\infty$-categories}

There are two primary ways to extend a cubical type theory with simplicial structure:

\begin{enumerate}
    \item \textbf{Synthetic $\infty$-categories (Riehl-Shulman 2017)}: Adds a dedicated simplicial interval, a face lattice for subshapes (topes), and extension types. This approach is highly expressive but requires significant kernel changes to handle a new class of dimensions and lattices.
    \item \textbf{Modal STT (Buchholtz et al. 2025)}: Leverages modal type theory to manage categorical variance and discreteness. It uses the "Flat Modality" ($\flat$) for discrete types and specialized modalities ($\sharp$, $\text{tw}$) for categorical operations.
\end{enumerate}

For a minimal extension of the \textbf{Anders} kernel, the \textbf{Modal STT} approach is preferred as it reuses the existing modal infrastructure (modal contexts, context restrictions, and variance tracking) instead of introducing a separate simplicial layer.

\subsubsection*{Minimal Constructors for STT}

The following primitive constructions are required for a minimal modal implementation of STT:

\begin{itemize}
    \item \textbf{Flat Modality} ($\flat$): The discrete modality, identifying types with no higher simplicial structure.
    \item \textbf{Sharp Modality} ($\sharp$): The modality for presheaf formation and hom-types.
    \item \textbf{Twisted Modality} ($\text{tw}$): Required for the twisted arrow category, essential for the Yoneda Lemma.
    \item \textbf{Modal $\Pi$-Types} ($\prod_{x :_\mu A} B$): Dependent functions that track categorical variance via modality $\mu$.
    \item \textbf{Modal Contexts}: Support for context extensions of the form $\Gamma, \{\mu\}$ and variable annotations $x :_\mu A$.
\end{itemize}

\newpage
\subsubsection{Canonical Equations for STT}

The STT extension introduces the following canonical reduction rules for the new type formers:

\paragraph{Modalities ($\sharp$, $\text{tw}$)}
The reduction behavior for the new modalities follows the standard modal pattern:
\begin{itemize}
    \item $\text{transp}^i (\sharp A) \phi (\sharp\text{-unit } a) = \sharp\text{-unit } (\text{transp}^i A \phi a)$
    \item $\text{hcomp}^i (\sharp A) [\phi \mapsto \sharp\text{-unit } u] (\sharp\text{-unit } u_0) = \sharp\text{-unit } (\text{hcomp}^i A [\phi \mapsto u] u_0)$
    \item $\text{transp}^i (\text{tw } A) \phi (\text{tw-unit } a) = \text{tw-unit } (\text{transp}^i A \phi a)$
    \item $\text{hcomp}^i (\text{tw } A) [\phi \mapsto \text{tw-unit } u] (\text{tw-unit } u_0) = \text{tw-unit } (\text{hcomp}^i A [\phi \mapsto u] u_0)$
\end{itemize}

\paragraph{Modal $\Pi$-Types}
The rules for modal $\Pi$-types $\prod_{x :_\mu A} B$ generalize the standard $\Pi$-type rules with modal context awareness:
\begin{itemize}
    \item $\text{transp}^i (\prod_{x :_\mu A} B) \phi u_0 v = \text{transp}^i B[x/w] \phi (u_0 w(i/0))$, where $w = \text{tFill}^{-i} (x :_\mu A) \phi v$.
    \item $\text{hcomp}^i (\prod_{x :_\mu A} B) [\phi \mapsto u] u_0 v = \text{hcomp}^i B[x/v] [\phi \mapsto u v] (u_0 v)$.
\end{itemize}
