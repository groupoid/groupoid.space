...
\begin{itemize}
\item \text{Fibrations} ($\mathbf{\Pi}$, $\mathbf{\Sigma}$)
\item \text{Identification Systems} ($\mathbf{=}$, $\mathbf{\equiv}$, $\mathbf{\sim}$, $\mathbf{\approx}$, $\mathbf{\simeq}$, $\mathbf{\cong}$, $\mathbf{Glue}$)
\item \text{Polynomials} ($\mathbf{W}$, $\mathbf{M}$, $\mathbf{HIT}$, $\mathbf{CoHIT}$)
\item \text{Modalities} ($\Im$, $\bigcirc$, $\flat$, $\sharp$)
\item \text{Localizations} ($\mathbf{Sp}$, $\mathbb{A}^1$, $\mathbb{\Delta}^n$, $\mathbb{Z}_2$)
\end{itemize}
