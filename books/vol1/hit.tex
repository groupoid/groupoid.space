...
\newpage
\subsection{Hub and Spokes Disc}

The hub and spokes construction, denoted \(\odot(S, A)\), is a higher inductive type that models the \((n)\)-truncation of a type. It can be thought of as a CW-complex-like structure consisting of a central \emph{hub} connected to points via \emph{spokes}.

\begin{definition}[Formation]
For any types \(S, A : \mathcal{U}\), the \textbf{hub and spokes} type is defined as
\[
\mathbf{disc}\, S\, A : \mathcal{U}.
\]
...
def spoke' (S A : U) (f : S → hs S A) (s : S)
  : PathP (<i> hs S A) (hub' S A f) (f s)
  := spoke S A f s
\end{lstlisting}
\end{definition}

\begin{theorem}[Elimination]
Let \(X : \mathsf{hs}\, S\, A \to \mathcal{U}\) be a type family. Given
\begin{align*}
  \mathsf{nCenter} & : \Pi (a : A),\ X(\mathsf{center}\, a), \\
  \mathsf{nHub}    & : \Pi (f : S \to \mathsf{hs}\, S\, A)\ 
                     (\mathsf{nF} : \Pi (s : S),\ X(f\, s)),\ 
                     X(\mathsf{hub}'\, f), \\
  \mathsf{nSpoke}  & : \Pi (f : S \to \mathsf{hs}\, S\, A)\ (\mathsf{nF} : \Pi (s : S),\ X(f\, s))\ (s : S), \\
  &\quad \mathsf{PathP}\ (<i>\ X(\mathsf{spoke}'\, f\, s\ @\ i))\ (\mathsf{nHub}\, f\, \mathsf{nF})\ (\mathsf{nF}\, s),
\end{align*}
there exists a function
\[
\mathsf{hs\text{-}ind}\, S\, A\, X\, \mathsf{nCenter}\, \mathsf{nHub}\, \mathsf{nSpoke} : \Pi (z : \mathsf{hs}\, S\, A),\ X(z)
\]
satisfying the computation rules below.

\[
\begin{cases}
\text{Ind}_\odot \, (\text{base} \, x) = \text{nCenter} \, x \\
\text{Ind}_\odot \, (\text{hub} \, f) = \text{nHub} \, f \\
\text{Ind}_\odot \, (\text{spoke} \, f \, s \, @ \, i) = \text{nSpoke} \, f \, s \, @ \, i \\
\end{cases}
\]
\end{theorem}
...
def hs-ind (S A : U) (X : hs S A → U)
    ...
    (z : hs S A) : X z
  := disc-ind S A X nCenter nHub nSpoke z
\end{lstlisting}

\begin{theorem}[Computation Rules]
The eliminator satisfies the following judgmental equalities:

\[
\begin{cases}
\mathsf{hs\text{-}ind}(\mathsf{center}\, a) \equiv \mathsf{nCenter}\, a \\
\mathsf{hs\text{-}ind}(\mathsf{hub}'\, f) \equiv \mathsf{nHub}\, f\ (\lambda s.\ \mathsf{hs\text{-}ind}(f\, s)) \\
\end{cases}
\]
...
def hs-β₂ (S A : U) (X : hs S A → U)
    ...
    (nSpoke : Π (f : S → hs S A) (nF : Π (s : S), X (f s)) (s : S),
              PathP (<i> X (spoke' S A f s @ i)) (nHub f nF) (nF s))
    (f : S → hs S A)
 : Path (X (hub' S A f)) (hs-ind S A X nCenter nHub nSpoke (hub' S A f)) 
   (nHub f (λ (s : S), hs-ind S A X nCenter nHub nSpoke (f s)))
:= idp (X (hub' S A f)) (nHub f (λ (s : S), hs-ind S A X nCenter nHub nSpoke (f s)))
\end{lstlisting}

\begin{theorem}[Uniqueness]
For any \(h : \Pi (z : \mathsf{hs}\, S\, A),\ X(z)\), we have
\[
h \equiv \mathsf{hs\text{-}ind}\, S\, A\, X\, (\lambda a.\ h(\mathsf{center}\, a))\, 
(\lambda f.\ \lambda \mathsf{nF}.\ h(\mathsf{hub}'\, f))\, \dots
\]
(i.e., the eliminator is unique up to homotopy).
\end{theorem}
