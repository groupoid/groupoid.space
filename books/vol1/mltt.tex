...
MLTT-75 and its complete internalization in \textbf{Per}, a minimal dependent type
...
eliminator for the first time for pure MLTT. This internalization serves as
...
Here = (definition), $\emptyset$ (empty set), $\rvert$, are parts of BNF language and
...
For MLTT-75, we focus on $\Pi$ (dependent function types), $\Sigma$ (dependent pair types),
...
of identity proofs (UIP) via an eta-rule. However, modern homotopical interpretations,
pioneered by Hofmann and Streicher \cite{Hofmann96}, refute UIP, adopting Path types
that model equality as paths in a space, aligning with cubical type theory’s constructive
framework. This shift is crucial for \textbf{Per}, as Path types facilitate the
internalization of MLTT-75’s rules.
...
The result of the work is a $\text{mltt.ctt}$ file which can be runned using $\mathbf{cubicaltt}$.
...
Along with $\mathbf{Alonso}$ cartesian interpreter this evaluators form a set of
