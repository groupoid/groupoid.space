...
for cubical type checkers is the direct embedding of type theory model into the language of type checker.
...
\begin{definition} (Type). Initial object of language AST inductive definition is called type.
\end{definition}

\begin{definition} (Term). Let {\bf C} be a type. Then term is an instance of {\bf C} or element of {\bf C}.
\end{definition}

\begin{definition} (Universe). The set of all types is called universe $\mathrm{U_0}$.
\end{definition}

\begin{definition} (SAR). The universum space is configured with a triple of:
i) sorts, a set of universes  $\mathrm{U_N}$ indexed over set N;
...
\begin{exercise} (CoC). SAR = $\{ \{\star , \Box \},\{ \star : \Box \},
        \{ \star \rightarrow \star : \star,
           \star \rightarrow \Box  : \Box,
           \Box  \rightarrow \star : \star,
           \Box  \rightarrow \Box  : \Box
        \}$. $\star$ represents terms, $\Box$ represents types.
\end{exercise}
