...
The goal of this article is to demonstrate the formal embedding of MLTT into MLTT$^{\nifty}$ with constructive proofs
...
algebras) and diagonal coersions could be made building most compact homotopy core.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{lccc}
    ...
    \hline
       ...
       AFH/ABCFHL, yacct, Arend & & x & $r \rightarrow s$\\
       HTS/CMS    & &   & $r \rightarrow s$, $weak$\\
  \end{tabular}
  \end{adjustbox}
\end{table}

The next langhuage after PTS$^{\infty}$ and MLTT$^{\infty}$ willl
be HTS$^{\infty}$ with recursive higher inductive type system and infinite number of universes.
Along with O-CPS interpreter this evaluators form a set of languages as a part of conceptual theorem
proving system with formalized virtual machine for extraction target.
...
\fontsize{8pt}{12pt}\selectfont \textrf{М.Е.Сохацький}\\
\vspace{0.1cm}
...
\indent {\bf Ключові слова:} Теорія типів Мартіна-Льофа, Кубічна теорія типів.
 ...
 \setmainfont{Arial}
    \vspace{1cm}
\fontsize{8pt}{12pt}\selectfont \textrf{М.Э.Сохацкий}\\
    \vspace{0.1cm}
