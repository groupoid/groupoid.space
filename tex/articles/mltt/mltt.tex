...
\begin{table}[h]
 ...
 \begin{tabular}{lcccc}
    ...
    \hline
       ...
       $\emptyset, \mathbf{1}$ & $\bot, \top$ & $\emptyset, \{ \emptyset \}$ & $\emptyset, *$\\
       ...
       $\prod{x:A},B(x)$ & $\forall_{x:A}B(x)$ & product & space of sections\\
       $\mathbf{Id}_{A}$ & equality $=$ & $\{ <x,x> \| x \in A \}$ & path space $A^I$ \\
      \hline
  \end{tabular}
