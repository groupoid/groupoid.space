...
\begin{lstlisting}[mathescape=true]
    [] := #list          mod    := ${\bf module}$ id ${\bf where}$ imp dec
     + := #sum           imp    := [ ${\bf import}$ id ]
     0 := #empty         tele   := 0 + cotele
   app := exp exp        cotele := ( exp : exp ) tele
    id := [ #nat ]       sum    := 0 + id tele + id tele | sum
   ids := [ id ]         br     := ids $\rightarrow$ exp
 codec := def dec        brs    := 0 + cobrs
   dec := 0 + codec      cobrs  := ${\bf |}$ br brs
   def := ${\bf data}$ id tele ${\bf =}$ sum + id tele : exp = exp + id tele : exp ${\bf where}$ def
   exp := ${\bf split}$ cobrs + ( exp ) + id + exp ${\bf .1}$ + exp ${\bf .2}$ + cotele * exp
         ${\bf \lambda}$ cotele $\rightarrow$ exp + cotele $\rightarrow$ exp + exp $\rightarrow$ exp + app
\end{lstlisting}
