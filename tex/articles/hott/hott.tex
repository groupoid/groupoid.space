...
be given in {\bf Issue IV: Higher Inductive Types}.
...
\begin{lstlisting}
iso_Form (A B: U): U = iso A B -> Path U A B
\end{lstlisting}
\end{definition}
...
\begin{lstlisting}
iso_Elim (A B: U): Path U A B -> iso A B
\end{lstlisting}
\end{definition}
...
\begin{lstlisting}
iso_Uniq (A B : U) (p: iso A B)
  : Path (iso_Form A B) (iso_Elim A B (iso_Intro A B p))
\end{lstlisting}
\end{definition}
