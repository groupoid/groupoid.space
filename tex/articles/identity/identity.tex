...
prover based on Cubical Type Theory (CTT) announced in 2017. We demonstrate the usage of this library by
showing how to build a constructive proof of heterogeneous equality, the simple
and elegant formulation of the equality problem, that was impossible to achieve in pure Martin-Löf Type Theory (MLTT).
...
\begin{figure}[h]
  ...
  \caption{Base Library used in Article}
\end{figure}
...
Thanks to Vladimir Voevodsky this was extended to Geometry, Topology and Homotopy Theory
using MLTT-based Coq proof assistant. He formulated the univalence principle $Iso(A,B)=(A=B)$,
however constructive proof that isomorphism equals to equality and equivalences is possible only
in Cubical Type Theory (Agda and Cubical type checkers).
...
\section{Informal Definition of Cubical Type Theory}
...
{\bf Heterogeneous Path}. The PathP formation rule defines a heterogenous path
...
de Morgan operation also could be used: $i \wedge j$, $i \vee j$, $-i$ and constants $0$ and $1$.
...
path from equivalences.

\section{The Formal Definition of Problem}

Let us have two types $A$ and $B$. And we have some theorems proved for $A$ and functions
$f: A -> B$ and $g: B -> A$ such that $f \circ g = id_A$ and $g \circ f = id_B$. Then we
...
\begin{figure}[h]
  ...
  \caption{Full Base Library for Homotopical Proving}
\end{figure}
